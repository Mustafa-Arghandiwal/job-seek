    ...
    return (
        <div className="relative grid grid-cols-[auto_1fr] grid-rows-[1fr_auto]  h-[calc(100dvh-138px-40px)] overflow-hidden px-1 xl:px-[150px] 2xl:px-[230px] ">
            ...
            <nav className={`text-nowrap relative flex flex-col pt-6  border-r border-r-customGray-100 ${isOpen ? 'w-[180px]' : 'w-16'} duration-300`}>
                ...
                {/* <button ref={openSidebarRef} onClick={() => setIsOpen(prev => !prev)} type="button" className={`z-20 bg-amber-400 rounded-full px-1 absolute right-0 translate-x-1/2 visible md:invisible`}>X</button> */}
                ...
                <Link href="/candidate/dashboard/applied-jobs" className={` ${url === '/candidate/dashboard/applied-jobs' ? 'before:h-full bg-primary-50 text-primary-500 hover:bg-primary-50 hover:text-primary-500 ' : 'before:h-0'} relative group before:absolute before:bg-primary-500 before:left-0 before:w-1 before:top-0   flex gap-4 items-center px-5 py-3 text-customGray-500 text-sm font-medium hover:text-customGray-900 hover:bg-customGray-50 before:rounded-4xl before:duration-150 before:ease-in-out `}>
                    ...
                    <span className={`${(!isOpen) && 'w-0 overflow-hidden'}`}>Applied Jobs</span>
                ...
                <Link href="/candidate/dashboard/favorite-jobs" className={` ${url === '/candidate/dashboard/favorite-jobs' ? 'before:h-full bg-primary-50 text-primary-500 hover:bg-primary-50 hover:text-primary-500 ' : 'before:h-0'} relative group before:absolute before:bg-primary-500 before:left-0 before:w-1 before:top-0   flex gap-4 items-center px-5 py-3 text-customGray-500 text-sm font-medium hover:text-customGray-900 hover:bg-customGray-50 before:rounded-4xl before:duration-150 before:ease-in-out `}>
                    ...
                    <span className={`${(!isOpen) && 'w-0 overflow-hidden'}`}>Favorite Jobs</span>
                ...
                <Link href="/candidate/dashboard/settings" className={` ${url === '/candidate/dashboard/settings' ? 'before:h-full bg-primary-50 text-primary-500 hover:bg-primary-50 hover:text-primary-500 ' : 'before:h-0'} relative group before:absolute before:bg-primary-500 before:left-0 before:w-1 before:top-0   flex gap-4 items-center px-5 py-3 text-customGray-500 text-sm font-medium hover:text-customGray-900 hover:bg-customGray-50 before:rounded-4xl before:duration-150 before:ease-in-out `}>
                    ...
                    <span className={`${(!isOpen) && 'w-0 overflow-hidden'}`}>Settings</span>
                ...
                <button onClick={() => router.post('/sign-out')}
                    ...
                    <span className={`${(!isOpen) && 'w-0 overflow-hidden'}`}>Logout</span>
            ...
            <main className="overflow-y-auto scrollbar-custom py-12 px-4 md:px-12 2xl:pr-32  ">
                {children}
            </main>
        </div>
    )
