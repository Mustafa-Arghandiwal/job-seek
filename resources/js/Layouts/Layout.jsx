        ...
        <div className="h-screen">
            <header className={`sticky top-0 bg-white shadow-lg  z-50 transition-transform duration-300 ${isVisible || dashboardUrls.includes(url) ? 'transform-none' : '-translate-y-full'}`}>
                <nav className="h-12 border-b border-b-customGray-50 flex justify-between items-center px-3 xl:px-24">
                    <ul className="text-customGray-600 text-sm gap-4 hidden md:flex ">
                        ...
                        <li><Link href="/jobs" className={`${(url === '/jobs' || url.startsWith('/jobs')) ? 'after:w-full text-primary-500' : 'after:w-0'} relative after:absolute after:bg-primary-500 after:left-1/2 after:-translate-x-1/2 after:bottom-0 after:h-0.5 pb-3.5 transition-all after:duration-200 after:ease-in-out`} >Find Job</Link></li>
                        <li><Link href="/employers" className={`${(url === '/employers' || url.startsWith('/employers?')) ? 'after:w-full text-primary-500' : 'after:w-0'} relative after:absolute after:bg-primary-500 after:left-1/2 after:-translate-x-1/2 after:bottom-0 after:h-0.5 pb-3.5 transition-all after:duration-200 after:ease-in-out`} >Find Employers</Link></li>
            ...
            <div ref={menuRef} className={`fixed z-50 shadow-lg bg-white w-full pb-6 px-6 rounded-b-2xl ${dropdownVisible ? "opacity-100 translate-y-0 visible" : "opacity-0 -translate-y-3 invisible"} transition-all duration-300 ease-in-out`}>
                <ul className="mt-1 text-sm text-customGray-600 w-full ">
                    ...
                    <li className="py-4 border-b border-b-customGray-100 hover:text-primary-500 duration-75"><Link href="/jobs">Find Job</Link></li>
