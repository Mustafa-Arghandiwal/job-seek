...
export default function Job(props) {
    ...
    const logo = props?.logo ? `/storage/${props.logo}` : null
        ...
        <Link href={`/vacancies/${props.id}`} className=" relative border border-primary-50 flex min-w-[260px]  sm:w-full justify-between gap-3 flex-wrap sm:flex-nowrap items-center p-8  rounded-xl
            ...
            <div className=" ">
                ...
                <div className=" flex gap-5 min-h-20 ">
                    {logo ?
                    <div style={{ backgroundImage: `url(${logo})` }} className="h-16 min-w-16 bg-cover bg-center rounded-md"></div>
                        :
                        <TextAvatar name={props.employerName} className="h-16 min-w-16 rounded-sm text-2xl" />
                    }
