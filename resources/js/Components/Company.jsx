...
export default function Company(props) {
    ...
    return (
        <div onClick={() => router.visit(`/employers/${props.id}`)}
            className="flex flex-col justify-between cursor-pointer   sm:gap-8 border h-[215px] sm:h-auto border-customGray-50 rounded-xl p-6 sm:p-8 max-w-[300px]
            sm:min-w-[290px]  hover:shadow-2xl hover:border-primary-500 duration-150 snap-center bg-white">
            <div className="flex gap-4">
                <div
                    className="h-16 min-w-16 bg-cover bg-center"
                    style={{ backgroundImage: `url(${props.logo ? "/storage/" + props.logo : '/chess_pattern.png'})` }}>
                </div>
