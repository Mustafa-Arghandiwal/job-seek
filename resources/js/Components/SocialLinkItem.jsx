    ...
    return (
        <div>
            ...
            <div className="flex flex-col relative xs:flex-row gap-1 xs:gap-3 mt-2 ">
                <div className="  w-full xs:w-5/6 max-w-[800px] rounded-md flex flex-col gap-2 xs:gap-0 xs:flex-row gap xs:border border-customGray-100">

                    <div className="relative xs:max-w-40 min-w-28 w-[calc(100%-52px)] xs:w-1/2 h-12 shrink-0  ">
                        <button type="button" ref={dropdownBtn} className={`w-full h-full  ${props.placeholder === '' ? 'text-customGray-400' : 'text-customGray-900'} ${dropdownVisible ? "ring-1 ring-primary-500" : "xs:rounded-none"}  flex justify-between px-3 items-center rounded-md  border xs:border-l-transparent xs:border-t-transparent xs:border-b-transparent xs:border-r border-customGray-100  gap-2 cursor-pointer`}
                ...
                <button type="button" className=" rounded-md bg-customGray-50 absolute right-0 top-0 xs:static   w-12 h-12 grid place-items-center cursor-pointer group hover:text-danger-500 duration-100 ">
                    {/* <img src="/Xcircle.png" /> */}
